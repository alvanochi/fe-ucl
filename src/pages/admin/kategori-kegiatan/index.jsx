...
export default function KategoriKegiatan() {
  /* eslint-disable */
  const { user } = useUser({ redirectTo: '/login' })
  const { prefix, menu, active, setActive } = useMenu()
  /* eslint-enable */

  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/kategori/kegiatan`
            ...
            {!loadingNew &&
              ...
              dataNew.map((row, index) => (
                <tr key={`row-${index}`}>
                  ...
                  <td className="text-sm border-2 border-white bg-gray-50 ">{row.nama_kegiatan}</td>
