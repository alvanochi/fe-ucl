...
export default function ChangePassword() {
  ...
  async function submitHandler(event) {
    ...
    try {
      const request = await axios({
        url: `${process.env.NEXT_PUBLIC_API_URL}/users/change-password/${router.query.id}`,
        method: 'PATCH',
  ...
  async function submitHandlerEmail(event) {
    ...
    try {
      const request = await axios({
        url: `${process.env.NEXT_PUBLIC_API_URL}/users/change-email/${router.query.id}`,
        method: 'PATCH',
        data: { email: email },
      })
      const response = await request
  ...
  const handleSwitchChange = async e => {
    ...
    try {
      const request = await axios({
        url: `${process.env.NEXT_PUBLIC_API_URL}/auth/verify-by-admin/${router.query.id}`,
        method: 'PUT',
  ...
  if ([user, menu, router.query.id].some(item => item == null || item == undefined))
    return <Loading />
  return (
    <Layout>
      <PageHeader title={`Ubah Sandi/Email ${menu.label}`} icon={menu.icon} handler={setActive} />
