...
export default function RekapKehadiran() {
    ...
    const fetchData = async () => {
      try {
        if (router.query.id) {
          ...
          const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL_ABSEN}/pembelajaran/list-absen`
  ...
  const GENERATE_URL = `${process.env.NEXT_PUBLIC_API_URL}/absensi/list-absensi/excel`
  ...
  return (
    <Layout>
      <PageHeader title={`Rekap Kehadiran Mahasiswa`} icon={menu.icon} handler={setActive} />
