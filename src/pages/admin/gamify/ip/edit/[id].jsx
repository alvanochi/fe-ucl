...
export default function DetailKategoriIp() {
  const router = useRouter()
  const { user } = useUser({ redirectTo: '/login' })
  const { prefix, menu, setActive } = useMenu()

  const API_URL = `${process.env.NEXT_PUBLIC_API_URL}/kategori/ip`
  ...
  const EDIT_URL = `${process.env.NEXT_PUBLIC_API_URL}/kategori/ip`
  ...
  return (
    <Layout>
      ...
      <Form onSubmit={event => submitHandler(event, EDIT_OPTION)}>
        <Card className="mt-4">
          <Card.Header className="text-center">Edit Kategori IP</Card.Header>
