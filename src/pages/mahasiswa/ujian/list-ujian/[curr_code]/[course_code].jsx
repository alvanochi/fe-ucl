...
export default function DetailUjian({ baseURL }) {
  ...
  const fetchData = async (currCode, courseCode) => {
    try {
      const API_URL = `${process.env.NEXT_PUBLIC_API_URL}/ujian`
  ...
  return (
    <Layout>
      <PageHeader title={`List ${menu.label}`} icon={menu.icon} handler={setActive} />
          ...
          data.map((row, index) => (
            ...
            >
              <div className="relative flex flex-col my-6 bg-white shadow-sm border border-slate-200 rounded-lg w-96 transition-transform duration-300 hover:shadow-lg hover:scale-105">
                ...
                >
                  <h3 className="text-center text-xl font-bold">{row.type_ujian}</h3>
                </div>

                <div className="p-4">
                  <h6 className="mb-2 text-slate-600 text-xl font-semibold">{row?.matkul?.name}</h6>
                ...
                <div className="flex items-center justify-between p-4">
                  <div className="flex items-center">
                    <div className="flex flex-col ml-3 text-sm">
                      <span className="text-slate-800 font-semibold">{row?.lecturer?.name}</span>
