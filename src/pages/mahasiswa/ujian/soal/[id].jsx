...
export default function Soal() {
  ...
  const fetchData = async (id) => {
    try {
      const API_URL = `${process.env.API_ENDPOINT}/ujian/soal`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const API_URL = `${process.env.API_ENDPOINT}/ujian/submit`;
                          ...
                          pgOptionsWithLetters.map((item, index) => (
                            <div key={index} className="mb-2">
                              <input
                                ...
                                value={item.letter}
                                checked={
                                  selectedAnswers[row.id] === item.letter
                                }
                                onChange={() =>
                                  handleAnswerChange(row.id, item.letter)
                                }
                                className="mr-2"
                              />
                              <label
                    ...
                    {row.type_soal === "ESAI" && (
                      <div>
                        <textarea
                          ...
                          onChange={(e) =>
                            handleEssayChange(row.id, e.target.value)
                          }
