...
export default function GenerateQrCode() {
  ...
  const { data: listMatkul, isLoading: isMatkulLoading } = useMatakuliah([user])
  ...
  async function submitHandler(event) {
    ...
    try {
      ...
      const request = await axios({
        url: `${process.env.NEXT_PUBLIC_API_URL}/absensi-external/pembelajaran`,
        method: 'POST',
        data: requestData,
      })
      ...
      try {
        if (selectedMatkul) {
          const response = await axios.get(
            `${process.env.NEXT_PUBLIC_API_URL}/absensi-external/pembelajaran/check-pertemuan`,
            {
              params: {
                id_dosen: data.user_id,
                id_matkul: selectedMatkul,
  ...
  if ([user, menu, loading, isMahasiswaLoading, isMatkulLoading].some(item => item == null))
    return <Loading />
  return (
    <Layout>
