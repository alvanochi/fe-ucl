...
const EditAbsensi = ({ data, onAddAbsensi }) => {
  ...
  const fetchAbsensiData = async (id) => {
    try {
      const response = await axios.get(`${process.env.API_ENDPOINT_ABSEN}/absensi-meeting`, {
        params: {
          filter: ["id"],
          filterValue: [id],
        },
  ...
  const submitHandler = async (event) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.API_ENDPOINT_ABSEN}/absensi-meeting/update/${data?.id || ""}`,
        formData
      );
      ...
      <Modal title="Edit Presensi" show={show} handler={toggle}>
        <Form className="space-y-4" onSubmit={submitHandler}>
          <Form.Group>
            ...
            <Form.Input type="text" name="name_absen" value={formData.name_absen} onChange={inputHandler} required />
          </Form.Group>
          <Form.Group>
            <Form.Label>
              NIP/NPM <span className="text-danger-600">*</span>
            </Form.Label>
            <Form.Input type="number" name="code" value={formData.code} onChange={inputHandler} required />
          </Form.Group>
