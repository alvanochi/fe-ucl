...
export default function DaftarHadirModule({ baseURL, user }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL_ABSEN}/pembelajaran`
  const DELETE_URL = `${process.env.NEXT_PUBLIC_API_URL_ABSEN}/pembelajaran/delete`
  ...
  const updateLearningDone = async id => {
    try {
      ...
      await axios.post(`https://absen.ft.uika-bogor.ac.id/api/pembelajaran/update/${id}`, {
                ...
                return (
                  <tr key={`row-${index}`}>
                    ...
                    <td className="text-sm border-2 border-white bg-gray-50">
                      {row.matkul?.name &&
                        row.matkul.name.split(' ').slice(0, 3).concat('...').join(' ')}
                    </td>
                    <td className="text-sm border-2 border-white bg-gray-50">{row.id_matkul}</td>
