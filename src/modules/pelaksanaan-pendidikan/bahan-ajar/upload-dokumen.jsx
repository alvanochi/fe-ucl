			...
			<Modal title="Unggah Dokumen" show={show} handler={toggle}>
				...
				<Form className="space-y-4">
					...
					<Form.Group>
						...
						<Form.Input type="text" name="keterangan" onChange={inputHandler} value={form.keterangan} />
					</Form.Group>
					<Form.Group>
						<Form.Label>
							Tautan Dokumen <span className="text-danger-600">*</span>
						</Form.Label>
						<Form.Input type="text" name="tautan_dok" onChange={inputHandler} value={form.tautan_dok} />
					</Form.Group>
					<div className="flex gap-4 mt-4">
						<Button type="button" onClick={toggle} variant="primary" className="w-full h-12">
							Konfirmasi
						</Button>
					</div>
				</Form>
