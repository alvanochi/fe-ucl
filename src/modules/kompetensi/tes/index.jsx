...
export default function TesModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/kompetensi/getTes`
  const DELETE_URL = `${process.env.NEXT_PUBLIC_API_URL}/kompetensi/deleteTes`
  ...
  const GENERATE_URL = `${process.env.NEXT_PUBLIC_API_URL}/skpi/bahasa`
                    ...
                    <div className="flex items-stretch gap-1">
                      ...
                      {(row.status === 0 || row.status === 2) && (
                        <>
                          <Button.Icon
                            onClick={() =>
                              window.open(`${baseURL}/tes/edit/${row.tes_id}`, '_blank')
                            }
                            variant="secondary"
