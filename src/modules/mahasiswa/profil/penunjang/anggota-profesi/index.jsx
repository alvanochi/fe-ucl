				...
				<thead>
					<tr>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("prof_id")}>
								No
								<SortIcon sort={getSortBy("prof_id")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("nama_organisasi")}>
								Nama Organisasi
								<SortIcon sort={getSortBy("nama_organisasi")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("peran")}>
								Peran/Kedudukan
								<SortIcon sort={getSortBy("peran")} />
							</div>
						</th>
						<th className="text-sm border-2 border-white bg-gray-200">
							<div className="flex items-center gap-2 cursor-pointer" onClick={() => sortBy("mulai_keanggotaan")}>
								Mulai Keanggotaan
					...
					{!loading &&
						...
						data.map((row, index) => (
							<tr key={`row-${index}`}>
								...
								<td className="text-sm border-2 border-white bg-gray-50">{row.nama_organisasi}</td>
