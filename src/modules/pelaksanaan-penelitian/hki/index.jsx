...
export default function HKIModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/hki/getDataHki`
  const DELETE_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/hki/deleteHki`
          ...
          {!loading &&
            ...
            data.map((row, index) => (
              <tr key={`row-${index}`}>
                ...
                <td className="text-sm border-2 border-white bg-gray-50">{row.judul_hki}</td>
                ...
                <td className="text-sm border-2 border-white bg-gray-50">
                  <div className="flex items-stretch gap-1">
                    <Button.Icon
                      onClick={() =>
                        window.open(`${`${baseURL}/hki/detail/${row.hki_id}`}`, '_blank')
                      }
                      variant="info"
