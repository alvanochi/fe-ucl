...
export default function PenelitianModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/getDataPenelitian`
  const DELETE_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/deletePenelitian`
      ...
      >
        <thead>
          <tr>
            ...
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Judul</div>
            </th>
            <th className="text-sm border-2 border-white bg-gray-200">
              <div
                className="flex items-center gap-2 cursor-pointer"
              ...
              <tr key={`row-${index}`}>
                ...
                <td className="text-sm border-2 border-white bg-gray-50">
                  <div className="flex items-stretch gap-1">
                    <Button.Icon
                      onClick={() =>
                        window.open(`${baseURL}/penelitian/detail/${row.penelitian_id}`, '_blank')
                      }
                      variant="info"
                    ...
                    {(row.status === 0 || row.status === 2) && (
                      <>
                        ...
                        <Button.Icon
                          onClick={() =>
                            window.open(`${baseURL}/penelitian/edit/${row.penelitian_id}`, '_blank')
                          }
                          variant="secondary"
