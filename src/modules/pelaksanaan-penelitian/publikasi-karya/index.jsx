...
export default function PublikasiKaryaModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/publikasi-karya/getPublikasi`
  const DELETE_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/publikasi-karya/deletePublikasi`
          ...
          {!loading &&
            ...
            data.map((row, index) => (
              <tr key={`row-${index}`}>
                ...
                <td className="text-sm border-2 border-white bg-gray-50">{row.judul_artikel}</td>
                ...
                <td className="text-sm border-2 border-white bg-gray-50">
                  <div className="flex items-stretch gap-1">
                    <Button.Icon
                      onClick={() =>
                        window.open(
                          `${baseURL}/publikasi-karya/detail/${row.publikasi_id}`,
                          '_blank',
                        )
                      }
                      variant="info"
                    ...
                    {(row.status === 0 || row.status === 2) && (
                      <>
                        ...
                        <Button.Icon
                          onClick={() =>
                            window.open(
                              `${baseURL}/publikasi-karya/edit/${row.publikasi_id}`,
                              '_blank',
                            )
                          }
                          variant="secondary"
                        ...
                        <Button.Icon
                          ...
                          onClick={() => destroy(row.publikasi_id).then(() => refresh())}
                        />
                      </>
                    )}
                  </div>
