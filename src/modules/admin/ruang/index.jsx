...
export default function RuanganModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/ruangan`
      ...
      >
        <thead>
          <tr>
            ...
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Alamat</div>
            </th>
          ...
          {!loadingNew &&
            ...
            dataNew.map((row, index) => (
              <tr key={`row-${index}`}>
                ...
                <td className="text-sm border-2 border-white bg-gray-50 ">{row.nama_ruangan}</td>
                <td className="text-sm border-2 border-white bg-gray-50 ">{row.alamat}</td>
