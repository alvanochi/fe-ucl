...
export default function AnggotaProfesiModule({ baseURL }) {
  const approveData = async id => {
    const UPDATE_URL = `${process.env.NEXT_PUBLIC_API_URL}/penunjang/approveStatusProf/${id}`
  ...
  const rejectData = async id => {
    const UPDATE_URL = `${process.env.NEXT_PUBLIC_API_URL}/penunjang/rejectStatusProf/${id}`
  ...
  const [dataUrl, setDataUrl] = useState(
    `${process.env.NEXT_PUBLIC_API_URL}/admin/anggotaProfPending`,
  )

  const handleApproveClick = async () => {
    await approveData()
    setDataUrl(`${process.env.NEXT_PUBLIC_API_URL}/admin/anggotaProfAprove`)
  }

  const handleRejectClick = async () => {
    await rejectData()
    setDataUrl(`${process.env.NEXT_PUBLIC_API_URL}/admin/anggotaProfReject`)
  }

  const handlePendingClick = () => {
    setDataUrl(`${process.env.NEXT_PUBLIC_API_URL}/admin/anggotaProfPending`)
  }
      ...
      <table className="w-full border-collapse rounded-2xl overflow-hidden shadow" cellPadding={10}>
        <thead>
          <tr>
            ...
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Nama Organisasi</div>
            </th>
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Peran/Kedudukan</div>
            </th>
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Periode</div>
            </th>
                      ...
                      {row.status === 0 && (
                        <>
                          <Button.Icon
                            ...
                            type="button"
                            icon={<Icon icon="oi:check" width={20} height={20} />}
                            onClick={() => approveData(row.prof_id).then(() => refresh())}
                          />
                          <Button.Icon
                            variant="danger"
                            type="button"
                            icon={<Icon icon="oi:x" width={20} height={20} />}
                            onClick={() => rejectData(row.prof_id).then(() => refresh())}
                          />
                        </>
                      )}
                    </div>
