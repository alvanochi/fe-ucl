...
import CreateKurikulum from './create'
import EditKurikulum from './edit'

export default function KurikulumModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/kategori/kurikulum`
    ...
    <>
      <div className="my-8">
        <div className="flex mb-8 justify-end items-center">
          <div className="mr-4">
            <CreateKurikulum onAction={handleAction} />
          </div>
            ...
            {!loadingNew &&
              ...
              dataNew.map((row, index) => (
                <tr key={`row-${index}`}>
                  ...
                  <td className="text-sm border-2 border-white bg-gray-50 ">{row.kurikulum}</td>

                  <td className="text-sm border-2 border-white bg-gray-50">
                    <div className="flex items-stretch gap-1">
                      <EditKurikulum id={row.id} onAction={handleAction} />
