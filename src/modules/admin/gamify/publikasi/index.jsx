...
export default function PublikasiGamifyModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/kategori/publikasi`
      ...
      <table className="w-full border-collapse rounded-2xl overflow-hidden shadow" cellPadding={10}>
        <thead>
          <tr>
            ...
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Nama Kategori</div>
            </th>
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Tingkat</div>
            </th>
            <th className="text-sm border-2 border-white bg-gray-200">
          ...
          {!loading &&
            ...
            data.map((row, index) => (
              <tr key={`row-${index}`}>
                ...
                <td className="text-sm border-2 border-white bg-gray-50">{row.nama_kategori}</td>
                <td className="text-sm border-2 border-white bg-gray-50">{row.tingkatan}</td>
                <td className="text-sm border-2 border-white bg-gray-50">{row.point}</td>
                <td className="text-sm border-2 border-white bg-gray-50">
                  <div className="flex items-stretch gap-1">
                    <Button.Icon
                      as="a"
                      href={`${baseURL}/publikasi/edit/${row.id}`}
                      variant="secondary"
