...
export default function PrestasiGamifyModule({ baseURL }) {
  const DATA_URL = `${process.env.NEXT_PUBLIC_API_URL}/kategori/prestasi`
      ...
      <table className="w-full border-collapse rounded-2xl overflow-hidden shadow" cellPadding={10}>
        <thead>
          <tr>
            ...
            <th className="text-sm border-2 border-white bg-gray-200">
              <div className="flex items-center gap-2 cursor-pointer">Juara</div>
            </th>
            <th className="text-sm border-2 border-white bg-gray-200">
          ...
          {!loading &&
            ...
            data.map((row, index) => (
              <tr key={`row-${index}`}>
                ...
                <td className="text-sm border-2 border-white bg-gray-50">{row.juara}</td>
