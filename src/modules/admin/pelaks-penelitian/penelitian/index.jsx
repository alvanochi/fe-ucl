...
export default function PenelitianModule({ baseURL }) {
  const approveData = async id => {
    const UPDATE_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/approveStatus/${id}`
  ...
  const rejectData = async id => {
    const UPDATE_URL = `${process.env.NEXT_PUBLIC_API_URL}/penelitian/rejectStatus/${id}`
  ...
  const [dataUrl, setDataUrl] = useState(
    `${process.env.NEXT_PUBLIC_API_URL}/admin/penelitianPending`,
  )

  const handleApproveClick = async () => {
    await approveData()
    setDataUrl(`${process.env.NEXT_PUBLIC_API_URL}/admin/penelitianAprove`)
  }

  const handleRejectClick = async () => {
    await rejectData()
    setDataUrl(`${process.env.NEXT_PUBLIC_API_URL}/admin/penelitianReject`)
  }

  const handlePendingClick = () => {
    setDataUrl(`${process.env.NEXT_PUBLIC_API_URL}/admin/penelitianPending`)
  }
          ...
          {!loading &&
            ...
            data.map((row, index) => (
              <tr key={`row-${index}`}>
                ...
                <td className="text-sm border-2 border-white bg-gray-50 ">{row.judul_kegiatan}</td>
                <td className="text-sm border-2 border-white bg-gray-50">
