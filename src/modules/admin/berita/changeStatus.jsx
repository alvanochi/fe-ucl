...
export default function ChangeStatus({ id, status, onStatusChange }) {
  ...
  const submitHandler = async event => {
    ...
    try {
      const EDIT_URL = `${process.env.NEXT_PUBLIC_API_URL}/berita/change-status/${form.id}`
    ...
    <>
      ...
      <Modal title="Change Status" show={show} handler={toggle}>
        <p className="text-center text-sm mb-8">
          Jika status suatu (Event / Tantangan) adalah Active maka informasi tersebut akan
          ditampilkan dalam daftar berita pada aplikasi mobile UCL. Sebaliknya, jika statusnya Non
          Active berarti informasi tersebut tidak akan ditampilkan.
        </p>
